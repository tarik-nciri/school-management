    ...
    if (isset($_POST['delet'])) {
        ...
        $id = $_POST['Tid'];
        $sql = $pdo->prepare("DELETE FROM teacherlist WHERE id = ? ");
    ...
    if (isset($_POST['update'])) {
        ...
        $TPass = $_POST['TPass'];
        $query = $pdo->prepare("UPDATE teacherlist set Tname = ? , TEmail = ? , TDOB = ? , Tsalary = ? , TPass = ? where id = ?");
        $result = $query->execute([$TName,$TEmail,$TDOB,$TSalary,$TPass,$id]);
        header('Location: index.php');
        }else {
            echo "Database Error: " ;
        }
        ...
        <div class="parent">
            <div class="left">
                <div class="w3-sidebar  w3-bar-block" style="width:16%; background-color:rgb(51, 58, 115);">
                    ...
                    <a href="login.php" style="color:white" class="w3-bar-item w3-button"><img src="picture.logout.png" width="25px" alt=""> Logout</a>
                </div>
            </div>
            <div class="rigth">
                <div class="container1">
                    <form action="" method="post">
                        ...
                        <div class="form-group">
                            <label for="TName">Teachers Name:</label>
                            <input type="text" id="TName" name="TName" required>
                        </div>
                        <div class="form-group">
                            <label for="TEmail">Teachers Email:</label>
                            <input type="email" id="TEmail" name="TEmail" required>
                        </div>
                        <div class="form-group">
                            <label for="TDOB">Teachers Date of Birth:</label>
                            <input type="date" id="TDOB" name="TDOB" required>
                        </div>
                        <div class="form-group">
                            <label for="TSalary">Teachers Salary:</label>
                            <input type="number" id="TSalary" name="TSalary" required>
                        </div>
                        <div class="form-group">
                            <label for="TPass">Teachers Password:</label>
                ...
                <div class="container2">
                    ...
                    if (isset($_POST['save'])) {
                        ...
                        $TSalary = $_POST['TSalary'];
                        $TPass = $_POST['TPass'];
                        $query = "INSERT INTO teacherlist (id,TName,TEmail,TDOB,TSalary,TPass) VALUES (null,'$TName','$TEmail','$TDOB','$TSalary','$TPass')";
                        if ($pdo->query($query)) {
                    ...
                    $content = "SELECT * from  teacherlist";
                    $result = $pdo->prepare($content);
                    $result->execute();
                    if ($result->rowcount() > 0) {
                        echo "<table border='1' id='tab'  width='5px'>
                    <thead>
                        <tr>
                            <th>ID </th>
                            <th> name</th>
